...
export function ButtonWithIcon({id, iconSrc, handleClick, width, height, className, alt, color = 'orange', disabled = false, label}: ButtonWithIconProps) {
    ...
    return (
        <button id={id} className={'rounded-lg disabled:opacity-50 ' + (disabled ? '' : colorMap.get(color))} onClick={handleClick} disabled={disabled}>
